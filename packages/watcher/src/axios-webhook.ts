...
const createAxiosInstance = () => {
  ...
  if (
    PODWATCH_CUSTOM_SERVER_URL &&
    PODWATCH_CLIENT_ID &&
    PODWATCH_CLIENT_SECRET
  ) {
    throw new Error(
      'We have detected both a custom server URL and a PodWatch Client ID and Secret. You will need one or the other to continue.'
    );
    process.exit(1);
  }
  //check if the podwatch custom server url exists
    ...
    throw new Error(
      'Your Custom Server URL, PodWatch Client ID and/or Secret are blank. Please review and try again.'
    );
    process.exit(1);
  } else {
